...
// ==================== 主渲染函数（已修复长按干扰点击） ====================
function render() {
  // 顶部 tab-bar 移除: 所有切换按钮都放在底部 dock
  ...
  html += '</div>';
  // 底部浮动 dock: 工具按钮挪到这里, 手机拇指范围内
  html += '<div class="dock-bar">';
  html += '<button class="dock-btn dock-save" id="tabSaveBtn" title="导出收益表"><span class="dock-ico">📊</span><span class="dock-lbl">导出</span></button>';
  html += '<button class="dock-btn dock-nav" id="refreshBtn" title="手动记录净值"><span class="dock-ico">📝</span><span class="dock-lbl">记净值</span></button>';
  html += '<button class="dock-btn dock-savefile" id="saveDataBtn" title="保存到文件"><span class="dock-ico">💾</span><span class="dock-lbl">保存</span></button>';
  html += '<button class="dock-btn dock-loadfile" id="loadDataBtn" title="从文件载入"><span class="dock-ico">📂</span><span class="dock-lbl">载入</span></button>';
  html += '</div>';
...
function updateRangeTrack(track) {
  ...
  var code = track.dataset.code;
  var f = null;
  if (code && typeof state !== "undefined") {
    f = state.find(function(x) { return x.code === code; });
  }
  var rate = 0;
  if (f) {
  ...
  if (runner) {
    ...
    if (rate >= 10) {
      ...
      if (emoji) emoji.textContent = String.fromCodePoint(0x1F3C3, 0x200D, 0x2640, 0xFE0F);
      if (dust) dust.textContent = String.fromCodePoint(0x1F4A8);
    } else if (rate >= 0) {
      runner.classList.add("running-slow");
      if (emoji) emoji.textContent = String.fromCodePoint(0x1F6B6, 0x200D, 0x2642, 0xFE0F);
      if (dust) dust.textContent = "";
    } else if (rate >= -10) {
      runner.classList.add("walking-back");
      if (emoji) emoji.textContent = String.fromCodePoint(0x1F61F);
      if (dust) dust.textContent = "";
    } else {
      runner.classList.add("running-flee");
      if (emoji) emoji.textContent = String.fromCodePoint(0x1F3C3, 0x200D, 0x2640, 0xFE0F);
      if (dust) dust.textContent = String.fromCodePoint(0x1F4A6);
    }
          ...
          return `
            ...
            <div class="range-bar-section">
              <div class="range-track"
                   ...
                   data-init-shares="${initShares}" data-base-price="${basePrice}" data-code="${code}">
